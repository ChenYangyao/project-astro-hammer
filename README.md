...
* 算法（Algorithm） - 如何实线程序的高效性
    ...
    我们会通过天文学里面的两个常见的例子：关联函数的计算和模板参数匹配，来展示算法对提高程序效率的重要作用。
...
* 并行技术基础
    ...
    任何并行计算都依赖于硬件和操作系统提供的环境。因而，对硬件和操作系统的基本了解是并行程序设计的基础。我们将介绍：多核系统的基本构架，操作系统中进程/线程的内存映像，进程的调度机制，进程/线程间数据共享的机制（共享内存、管道、进程旗语、socket、消息队列、文件），进程/线程间的互斥与同步机制（原语、自旋、信号量、锁和事务）。我们将通过具体的例子来介绍并行算法设计的特点和方法（进程/线程的启动与合并，同步与互斥机制，任务分配与均衡，数据共享和进程通信）。

* 并行控制协议和软件简介

    并行程序设计依赖于操作系统提供的并行控制协议和软件。我们将介绍linux系统下的共享内存的并发技术-pthread，以及消息传递的并发技术-linux 进程。这些基本的技术允许我们对并行程序以及它们之间的相互作用提供细粒度的控制。
    
    一些高级的协议和接口，允许我们更方便的利用并行环境。我们将介绍两个高级的接口：OpenMP-共享内存的并发技术，和MPI-消息传递接口。这些高级接口使用更加方便，允许我们对并发程序进行粗粒度的控制。
